...
public class SuperSettingsActivity extends BaseFragment {
    ...
    public View createView(Context context) {
        ...
        actionBar.setTitle(LocaleController.getString(R.string.SUPERSETTINGS));
        ...
        listView.setOnItemClickListener((view, position, x, y) -> {
            ...
            if (item.viewType == VIEW_TYPE_SWITCH2) {
                if (item.type == SWITCH_TURNDOWN) {
                    SharedPreferences preferences = MessagesController.getGlobalMainSettings();
                    boolean turndown = preferences.getBoolean("turndown", false);
                    SharedPreferences.Editor editor = preferences.edit();
                    editor.putBoolean("turndown", !turndown);
                    SharedConfig.setAnimationsEnabled(!turndown);
                    editor.commit();
                    ((TextCell) view).setChecked(!turndown);
                }
            }
        });
    ...
    private void updateItems() {
        ...
        items.add(Item.asHeader(LocaleController.getString("HAHAHA")));
        items.add(Item.asInfo(""));

        items.add(Item.asSwitch(LocaleController.getString("TURNTHEMDOWN"), SWITCH_TURNDOWN));
        items.add(Item.asInfo(LocaleController.getString("TURNTHEMDOWNINFO")));
    ...
    public static final int SWITCH_TURNDOWN = 1;
    ...
    private class Adapter extends AdapterWithDiffUtils {
        ...
        public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
            ...
            final SuperSettingsActivity.Item item = items.get(position);
...
//                updateSlider(slideChooseView);
                ...
                if (item.type == SWITCH_TURNDOWN) {
                    SharedPreferences preferences = MessagesController.getGlobalMainSettings();
                    boolean turndown = preferences.getBoolean("turndown", true);
                    textCell.setTextAndCheck(item.text, turndown, false);
                }
            }
        }
